// src/pages/WorldSelector/index.jsx
...
import {
  ...
  WorldInfo, LoadingText, NoWorldsText, DeleteButton,
} from './styles';
...
const WorldSelector = ({ onLogout }) => {
    ...
    const handleWorldCreated = (newWorld) => {
        ...
        // 2. Lógica de ID simplificada
        const createdId = newWorld?._id; 
        if (createdId) {
            navigate(`/simulation/${createdId}`);
        } else {
            console.warn('ID do mundo criado não encontrado:', newWorld);
        }
    };

    const handleDeleteWorld = async (id, e) => {
        e.stopPropagation();
        if (!window.confirm('Deseja realmente deletar este mundo?')) return;
        try {
            await api.delete(`/worlds/${id}`);
            setWorlds(prev => prev.filter(w => w._id !== id));
        } catch (error) {
            console.error('Erro ao deletar mundo:', error);
            alert('Falha ao deletar o mundo.');
        }
    };
            ...
            {/* ... O resto do JSX permanece o mesmo ... */}
             <WorldListContainer>
                {worlds.length > 0 ? (
                    <WorldList>
                        {worlds.map(world => (
                            <WorldCard key={world._id} onClick={() => navigate(`/simulation/${world._id}`)}>
                                <DeleteButton onClick={(e) => handleDeleteWorld(world._id, e)} title="Deletar mundo">
